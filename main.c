...
File: main.c
Program: line_processor
Resources: Threads Explorations, Assignment Example Program, Assignment 3
*/
#include <stdlib.h>
#include <stdio.h>
#include <pthread.h>
...
void *input_thread(void *args){
    ...
    for(int i=0; i < NUM_ITEMS; i++){
        ...
        //Increment producer index and counter
        count_1++;
        prod_idx_1++;
        ...
        //check for stop
        if(strcmp(input_buffer1, stop) == 0) break;
    }
    return NULL;
}
...
void *line_separator_thread(void *args){
    ...
    for(int j=0; j < NUM_ITEMS; j++){
        ...
        strcpy(input_buffer2, buffer_1[con_idx_1]);
        ...
        //Replace \n with space if not stop phrase
        if(strcmp(input_buffer2, stop) != 0) input_buffer2[strlen(input_buffer2)-1] = ' ';

        //Produce
        //Lock buffer_2
        pthread_mutex_lock(&mutex_2);
        //Put into buffer_2
        strcpy(buffer_2[prod_idx_2], input_buffer2);
        //Increment counters
        prod_idx_2++;
        count_2++;
        //Signal that buffer is no longer empty
        ...
        if(strcmp(input_buffer2, stop) == 0) break;
    }
    return NULL;
}
...
void *plus_sign_thread(void *args){
    ...
    for(int k=0; k<NUM_ITEMS; k++){
        ...
        //Search string for ++
        char replace = '^';
        for(char *subStringPtr = strstr(input_buffer3, "++"); subStringPtr != NULL; subStringPtr = strstr(input_buffer3, "++")){
            ...
            if(strlen(input_buffer3) - size > 2) strncat(searchBuffer, subStringPtr+2, strlen(input_buffer3) - size);
